...
| Item | Impact | Status |
|---|---|---|
| No payment rail for Nepal | Stripe cannot take most Nepali cards, so the one payment method that exists serves the diaspora and not Kathmandu — in a product positioned around Nepal | OPEN. eSewa and Khalti are the actual gap. The `BillingProvider` interface already abstracts this, so a second provider is an implementation, not a redesign |
