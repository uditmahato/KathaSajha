...
| Item | Impact | Status |
|---|---|---|
| Illustrations need Google Cloud billing | Every image model — `gemini-2.5-flash-image`, `gemini-3.1-flash-image`, `gemini-3.1-flash-lite-image`, `gemini-3-pro-image` — returns 429 with `limit: 0` on the free tier. Not "quota used up": image generation is unavailable on free, so stories render with "illustration unavailable" on every page. Text generation works on a free key | OPEN. Owner action: enable billing on the Cloud project. `IMAGE_PROVIDER=mock` renders local placeholder art meanwhile (see `.claude/launch.json` -> `kathasajha-real-stories`) |
| Cost per story is still unknown | Text is measured and trivial — a Nepali story is ~490 in / ~490 out tokens, a fraction of a cent. Images are the cost driver and cannot be measured until billing is on, and `PRICE_PER_*` are still unset so every generation logs $0 | PARTIAL. Half the answer now exists |
