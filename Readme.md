...
Clone it, fork it, run it, change it — the setup is two commands and needs no API key, because the
mock provider exercises the whole system at zero cost.

```bash
cp .env.example .env && docker compose up --build
```

[CONTRIBUTING.md](CONTRIBUTING.md) covers development setup, how to run the suite, and the house
rules — most of which exist because something went wrong once, with the incident recorded in
[docs/LESSONS.md](docs/LESSONS.md). Two are worth knowing before your first test run:

- **On Windows, never run `pytest` bare.** A bare run once reached 58 GB and took the machine
  down. Use the memory-capped wrapper, which measures the whole process tree.
- **Scan for self-referential monkeypatches first.** `lambda *a: mod.f(...)` patched over `mod.f`
  is infinite recursion, not delegation.
...
[GNU Affero General Public License v3.0](LICENSE) — copyright © 2026 Udit Mahato.

You may use, study, modify and redistribute this software. The obligation that distinguishes AGPL
from ordinary GPL is **section 13**: if you run a *modified* version as a network service that
other people use, you must offer those users the source of your modified version. Running an
unmodified copy, or running a modified copy privately, carries no such duty.

Chosen deliberately. This is intended to be genuinely open — forkable, readable, and usable for
learning or self-hosting — while making it unattractive to take the work, close it, and sell a
competing hosted service against the project it came from. Contributions are accepted under the
same licence.

The licence covers the source. It does not grant rights to the KathaSajha name or branding, and
stories generated by the service belong to the families who create them.
