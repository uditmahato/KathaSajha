...
(function () {
    ...
    // Subresource integrity: without it a hijacked CDN response would run with
    // full page privileges and could read the token out of localStorage. The
    // browser refuses to execute anything that does not hash to this exactly.
    const PDF_LIB_SRI = 'sha512-GsLlZN/3F2ErC5ifS5QtgpiJtWd43JWSuIgh7mbzZ8zBps+dvLusV+eNQATqgA/HdeKFVgA5v3S/cIrLF7QnIg==';
    let pdfLibPromise = null;

    /** Load the PDF library only when someone actually saves a PDF. */
    function loadPdfLib() {
        ...
        if (!pdfLibPromise) {
            pdfLibPromise = new Promise((resolve, reject) => {
                ...
                s.onload = () => resolve();
                s.onerror = () => { pdfLibPromise = null; reject(new Error('Could not load the PDF tool. Please check your connection and try again.')); };
    ...
    els.pdfBtn.addEventListener('click', async () => {
        ...
        els.pdfBtn.textContent = 'Preparing...';
        try {
            await loadPdfLib();
        } catch (err) {
            toast(err.message, 'error');
        ...
        coverTitle.textContent = els.storyTitle.textContent;
        const byline = document.createElement('p');
        byline.className = 'cover-byline';
        byline.textContent = 'Generated by KathaSajha';
        cover.append(coverTitle, byline);
        root.appendChild(cover);

        renderPages(root, currentStory.pages, { language: currentStory.language });
        root.querySelectorAll('.story-page').forEach((p) => { p.style.marginBottom = '0'; });

        const filename = (els.storyTitle.textContent.replace(/[^a-z0-9ऀ-ॿ]/gi, '_') || 'story') + '.pdf';
        html2pdf().set({
            margin: 0,
            filename: filename,
            ...
            pagebreak: { mode: ['css'], avoid: ['img', '.text'] },
        }).from(root).save();
    });
